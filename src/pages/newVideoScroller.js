...
const Card = ({
    ...
    const [voicesLoaded, setVoicesLoaded] = useState(false); // Declare voicesLoaded state
    ...
    useEffect(() => {
      const loadVoices = () => {
        ...
        if (voices.length) {
          ...
          console.warn("Voices are still loading...");
        }
      };
    ...
    // Function to handle starting speech for the current card
    const handleStartSpeaking = () => {
      ...
      if (availableVoices.length > 0) {
        utterance.voice = availableVoices.find(voice => voice.name === 'Google US English') || availableVoices[0];
      } else {
      ...
      utterance.onend = () => {
        console.log("Speech ended for:", currentText);
      };
    ...
    // Handle card entering view and cancel previous speech
    useEffect(() => {
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              if (stopPreviousSpeech) stopPreviousSpeech(); // Cancel previous card speech
              handleStartSpeaking(); // Start speaking for this card
            }
          });
        },
        { threshold: 0.5 }
    ...
    // Handle speech synthesis for the current card
    useEffect(() => {
      ...
      // Set a male voice (adjust as needed)
      ...
      utterance.onend = () => {
        if (currentTextIndex < data.card.length - 1) {
          ...
          if (cardRef.current.nextSibling) {
            // Automatically start next card if available
            cardRef.current.nextSibling.scrollIntoView({ behavior: "smooth" });
          }
        }
      };
  
      return () => {
        synth.cancel(); // Cleanup speech on component unmount
      };
    }, [isSpeaking, currentTextIndex, voicesLoaded, data, setIsSpeakingGlobal]);
